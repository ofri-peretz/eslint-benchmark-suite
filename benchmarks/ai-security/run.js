...
import { PROMPTS, MODELS, RULE_CATEGORY_MAP, RULE_METADATA, DEFAULT_CONFIG } from './prompts.js';
...
function chiSquaredTest(modelData) {
  ...
  if (models.length < 2) return { chiSquared: 0, df: 0, pValue: 1, significant: false };

  const totalSuccesses = models.reduce((s, m) => s + modelData[m].successes, 0);
  const totalFailures = models.reduce((s, m) => s + (modelData[m].total - modelData[m].successes), 0);
  const grandTotal = totalSuccesses + totalFailures;

  let chiSq = 0;
  models.forEach((m) => {
    const observed = [modelData[m].successes, modelData[m].total - modelData[m].successes];
    const expected = [
      (modelData[m].total * totalSuccesses) / grandTotal,
      (modelData[m].total * totalFailures) / grandTotal,
    ];
    chiSq += observed.reduce((s, o, i) => s + Math.pow(o - expected[i], 2) / expected[i], 0);
  });
  ...
  try {
    ...
    for (const result of results) {
      for (const message of result.messages) {
        ...
        const category = metadata.category || RULE_CATEGORY_MAP[message.ruleId] || 'other';

        // Extract the source line for context
        const sourceLine = message.line > 0 && message.line <= codeLines.length
...
async function runBenchmark(config = DEFAULT_CONFIG) {
  ...
  for (const modelName of config.models) {
          ...
          try {
            ...
            if (violations.length > 0) {
              ...
              for (const v of violations) {
                ...
                if (v.cwe) {
                  modelResults.byCWE[v.cwe] = modelResults.byCWE[v.cwe] || { count: 0, name: v.cweName };
                ...
                if (v.owasp) {
                  modelResults.byOWASP[v.owasp] = (modelResults.byOWASP[v.owasp] || 0) + 1;
                }
                ...
                if (v.ruleId) {
                  modelResults.byRule[v.ruleId] = modelResults.byRule[v.ruleId] || {
                    count: 0,
                    cwe: v.cwe,
                    cvss: v.cvss,
            ...
            if (violations.length > 0) {
              try {
                const fixedCode = await remediateWithClaude(code, violations, modelConfig.model);
                ...
                const fixRate = originalCount > 0
                  ? Math.round(((originalCount - fixedCount) / originalCount) * 100)
                  : 100;

                // Track remediation stats
                modelResults.remediationAttempts = (modelResults.remediationAttempts || 0) + 1;
                modelResults.remediationFullyFixed = (modelResults.remediationFullyFixed || 0) + (fixedAll ? 1 : 0);
                modelResults.remediationTotalOriginal = (modelResults.remediationTotalOriginal || 0) + originalCount;
                modelResults.remediationTotalRemaining = (modelResults.remediationTotalRemaining || 0) + fixedCount;

                // Store remediation results
                promptResults.iterations[promptResults.iterations.length - 1].remediation = {
                  fixedCode,
    ...
    const vulnRate = modelResults.totalFunctions > 0
      ? (modelResults.functionsWithVulnerabilities / modelResults.totalFunctions) * 100
      : 0;
    ...
    modelResults.averageCVSS = modelResults.cvssCount > 0
      ? Math.round((modelResults.totalCVSS / modelResults.cvssCount) * 10) / 10
      : null;

    // Calculate remediation effectiveness (static analysis as Guardian Layer)
    if (modelResults.remediationAttempts > 0) {
      const fixRate = modelResults.remediationTotalOriginal > 0
        ? Math.round(((modelResults.remediationTotalOriginal - modelResults.remediationTotalRemaining) / modelResults.remediationTotalOriginal) * 100)
        : 0;
    ...
    console.log(`   With vulnerabilities: ${modelResults.functionsWithVulnerabilities}`);
    ...
    if (modelResults.remediationEffectiveness) {
      ...
      console.log(`      Fully fixed: ${modelResults.remediationEffectiveness.fullyFixed}/${modelResults.remediationEffectiveness.attempts} (${modelResults.remediationEffectiveness.fullyFixedRate})`);
      console.log(`      Overall fix rate: ${modelResults.remediationEffectiveness.overallFixRate}`);
    }
  }

  // Generate cross-model insights for the benchmark summary
  const modelNames = Object.keys(results.models);
  const allVulnRates = modelNames.map(m => results.models[m].vulnerabilityRateNumeric);
  const allCVSS = modelNames.map(m => results.models[m].averageCVSS).filter(Boolean);
  ...
  results.summary = {
    ...
    totalFunctions: Object.values(results.models).reduce((sum, m) => sum + m.totalFunctions, 0),
    totalVulnerabilities: Object.values(results.models).reduce((sum, m) => sum + m.totalVulnerabilities, 0),
    ...
    averageCVSS: allCVSS.length > 0
      ? Math.round((allCVSS.reduce((a, b) => a + b, 0) / allCVSS.length) * 10) / 10
      : null,
    // Collect unique CWEs across all models
    allUniqueCWEs: [...new Set(modelNames.flatMap(m => results.models[m].uniqueCWEs))],
    // Top categories across all models
    topCategories: Object.entries(
      modelNames.reduce((acc, m) => {
        for (const [cat, count] of Object.entries(results.models[m].byCategory)) {
...
function printSummary(results) {
  ...
  if (results.summary) {
    console.log('\n📊 OVERALL FINDINGS:');
    console.log(`   Total functions analyzed: ${results.summary.totalFunctions}`);
    console.log(`   Total vulnerabilities found: ${results.summary.totalVulnerabilities}`);
    console.log(`   Average vulnerability rate: ${results.summary.averageVulnerabilityRate}`);
    console.log(`   Vulnerability rate range: ${results.summary.vulnerabilityRateRange}`);
    if (results.summary.averageCVSS) {
      console.log(`   Average CVSS score: ${results.summary.averageCVSS}/10`);
    }
    console.log(`   Unique CWEs detected: ${results.summary.allUniqueCWEs?.length || 0}`);
  }

  // Model comparison table
  console.log('\n📈 MODEL COMPARISON:');
  console.log('| Model | Functions | Vulnerable | Rate | Avg CVSS | Unique CWEs |');
  console.log('|-------|-----------|------------|------|----------|-------------|');
...
const iterationsArg = parseInt(
  args.find((a) => a.startsWith('--iterations='))?.split('=')[1] || DEFAULT_CONFIG.iterationsPerPrompt
);
